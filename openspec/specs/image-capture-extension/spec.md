...
Define the image capture and interception behavior of the Chrome Extension, using Background Service Worker debugger/CDP Network capture as the primary interception mechanism and Chrome Side Panel as the primary control surface.
...
### Requirement: 图片过滤
扩展必须 (SHALL) 支持 CDP 响应阶段的图片过滤规则，在拦截时（下载前）执行过滤判断。

**过滤执行时机**:
- 在 CDP Network.responseReceived 阶段执行过滤（body 提取前）
- 使用响应头和 URL 信息判断，无需下载完整内容
- 可用的拦截时信息：
  - URL（域名、文件名、扩展名）
...
- **BMP**: `image/bmp`
- **TIFF**: `image/tiff`
- **SVG**: `image/svg+xml` - **完全不支持，跳过处理**
- **ICO**: `image/x-icon` / `image/vnd.microsoft.icon`

**SVG 处理策略**:
- **检测**: 当检测到 `Content-Type: image/svg+xml` 时
- **处理**: 直接标记为"已跳过（SVG）"，不调用 `Network.getResponseBody` 也不发送到 VFS
- **统计**: 在统计中增加"跳过的 SVG 数量"

#### Scenario: 域名白名单（可选）
- **WHEN** 用户指定允许的域名
- **THEN** 扩展必须 (SHALL) 仅捕获来自这些域名的图片
- **AND** 未配置白名单时捕获所有域名的图片
